...
__all__ = ['ResNet_IBN_Cluster', 'resnet50_ibn_a_cluster' ]
    # , 'resnet101_ibn_a', 'resnet152_ibn_a', 'se_resnet101_ibn_a']
...
class Bottleneck_IBN_Cluster(nn.Module):
    expansion = 4
...
class ResNet_IBN_Cluster(nn.Module):

    def __init__(self, last_stride, block, layers, num_classes=1000):
        scale = 64
        self.inplanes = scale
        super(ResNet_IBN_Cluster, self).__init__()
...
def resnet50_ibn_a_cluster(last_stride, pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_IBN_Cluster(last_stride, Bottleneck_IBN_Cluster, [3, 4, 6, 3], **kwargs)
    if pretrained:
